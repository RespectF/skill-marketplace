...
async function testGenerateUiConfig() {
  const skillMd = `# Seedance Prompt Generator
Generate high-quality video prompts for Seedance AI video generation.

## Features
- Create cinematic video descriptions
- Support multiple styles and moods
...
请根据 Skill 的实际功能来设计，不要千篇一律。`,
        ...
        {
          role: "user",
          content: `Skill 名称：seedance 提示词改写
Skill 描述：Generate high-quality video prompts for Seedance AI
SKILL.md 内容：
${skillMd}
...
请生成个性化 UI 配置 JSON：`,
    ...
    console.log("Raw content:", content?.slice(0, 50));
    ...
    console.log("After stripping markdown:", jsonStr?.slice(0, 50));
    
    if (jsonStr) {
      try {
        const parsed = JSON.parse(jsonStr);
        console.log("\nParsed successfully!");
        console.log("Has features:", parsed.features?.length);
        console.log("Has useCases:", parsed.useCases?.length);
      } catch (e: any) {
